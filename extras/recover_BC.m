...
q := 5;
F := FiniteField(q);
vars_num := 2*n^2; //only need *n^2 since we assume we know A
...
Grobner_withA := function(pk,b,fake_comp,A)
...
// we then solve it using Grobner
	...
	B := Matrix(n,[X[i]:i in [1..n^2]]]);
	C := Matrix(n,[X[i]:i in [(n^2 + 1)..(2*n^2)]]);
...
A,B,C,b,pk := keygen();
D := Matrix(n,[X[i]:i in [(n^2 + 1)..(2*n^2)]]);
fake_comp := GACE(A,B,D*C,b); // public key times variables we are looking for in C
//"solutions";
S := Grobner_withA(pk,b,fake_comp,A); // we give A as input as we assumed we recovered it with minrank
